...
(function (global, factory) {
    ...
    function getCircleMesh(shape, totalSegments = 40, vertices, indices, angle)
    {
        vertices = vertices || new Float32Array((totalSegments + 1) * 2);
        indices = indices || new Uint16Array(totalSegments + 1);

        let circleAngle = (Math.PI * 2);
        if (angle) {
            circleAngle = angle;
        }
        const seg = circleAngle / totalSegments;
        let indicesIndex = -1;
...
`;
    ...
    {
        constructor(color = 0xFFFFFF, brightness = 1, radius = Infinity, angle = 0)
        {
            if (angle) {
                const shape = new math.Circle(0, 0, radius);
                const { vertices, indices } = getCircleMesh(shape, undefined, undefined, undefined, angle);
